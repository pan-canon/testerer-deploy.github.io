const chatLocales_en = {
  // Initial greeting from the spirit (chat bot)
  "greeting": "Hello! How can I help you today?",
  // Prompt for the user to select one of the answer options
  "response_prompt": "Please choose one of the following options:",
  // Answer options – you can add or remove options as needed
  "option_1": "Tell me more about this.",
  "option_2": "I don't understand.",
  "option_3": "Ignore.",
  // Message prefixes for alignment in the chat interface
  "user_message_prefix": "You: ",
  "spirit_message_prefix": "Spirit: ",
  // End-of-conversation message
  "end_of_conversation": "Thank you for chatting!"
};

export default chatLocales_en;