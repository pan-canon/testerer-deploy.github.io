...
export class BaseRepeatingQuest extends BaseEvent {
  ...
  async finishStage() {
    ...
    // First, add a diary entry for the photo attached.
    ...
    // Then, add a separate diary entry for the stage completion.
    ...
    if (this.currentStage <= this.totalStages) {
      // For intermediate stages, update the quest record to "finished" so that a new activation is required.
      await this.app.databaseManager.saveQuestRecord({
        quest_key: this.key,
        status: "finished",
        current_stage: this.currentStage,
      ...
      // If the current stage exceeds total stages, finish the quest completely.
